	...
	public void run() {
		...
		while (true) {
			...
			synchronized (lock) {
				...
				dLeftWheel = Math.PI * NavigationLab.WHEEL_RADIUS * (leftMotorTachoCount - leftLastTachoCount) / 180;
				dRightWheel = Math.PI * NavigationLab.WHEEL_RADIUS * (rightMotorTachoCount - rightLastTachoCount) / 180;


				deltaD = (dLeftWheel + dRightWheel)/2; // magnitude of displacement
				deltaT = (dLeftWheel - dRightWheel)/NavigationLab.TRACK; // change in heading
